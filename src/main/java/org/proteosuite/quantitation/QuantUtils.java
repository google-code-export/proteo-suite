...
public class QuantUtils {
	...
	public static String writeMzQuantML(String technique, String techniqueVariation, String quantFileName,
		...
		for (int iI = 0; iI < rawData.size(); iI++) {
			// Identification files
			IdentDataFile identFile = rawData.get(iI)
			...
			// Saving map for AssayList
			idFileNameIdMap.put(idFname, idId);
		}
		...
		if (technique.contains("iTRAQ")) {
			String[] sPipeline = PluginManager.getPlugins(technique, rawData
					.get(0).getFormat(), rawData.get(0)
					.getIdentificationDataFile().getFormat(), "mzQuantML");
			...
			up3_3.setValue(directory.replace("\\", File.separator)
					+ File.separator + "xTracker_" + sPipeline[2] + ".xtp");
			pmList3.add(up3_3);
			...
			up4_3.setValue(directory.replace("\\", File.separator)
					+ File.separator + "xTracker_" + sPipeline[3] + ".xtp");
			pmList4.add(up4_3);
		...
		if (technique.contains("emPAI")) {
			...
			UserParam up3 = new UserParam();
			up3.setName("Plugin configuration file");
			up3.setValue(directory.replace("\\", File.separator)
					+ File.separator + "xTracker_" + sPipeline[0] + ".xtp");
			pmList.add(up3);
			dataProcessing.getProcessingMethod().add(processingMethod);
		...
		MzQuantMLMarshaller marshaller = new MzQuantMLMarshaller(directory
				+ File.separator + quantFileName);
