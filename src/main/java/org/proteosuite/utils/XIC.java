...
public class XIC {
	...
	public XIC(String xmlFile, double rtStarts, double rtEnds, double mzStarts,
			double mzEnds) {
		...
		while (spectrumIterator.hasNext()) {
			...
			if (mslevel.equals("1")) {
				...
				double rt = 0.0;
				String unitRT = "";
				List<CVParam> scanParam = spectrum.getScanList().getScan()
						.get(0).getCvParam();
				for (Iterator<CVParam> lCVParamIterator = scanParam.iterator(); lCVParamIterator
					...
					if (lCVParam.getAccession().equals("MS:1000016")) {
						...
						if (unitRT.equals("UO:0000031")) // Validating rt unit
															// (mins or secs)
						{
				...
				if (rt >= rtStarts && rt <= rtEnds) {
					...
					if (mzNumbers != null) {
						for (int iI = 0; iI < mzNumbers.length; iI++) {
							if (mzNumbers[iI].doubleValue() >= mzStarts
									&& mzNumbers[iI].doubleValue() <= mzEnds) {
