 ...
 * --------------------------------------------------------------------------
 * Description: Utility to trim zero intensity pairs. Developer: SPerkins
 * Created: 26 November 2013 Read our documentation file under our Google SVN
 * repository SVN: http://code.google.com/p/proteo-suite/ Project Website:
    ...
    private void trimZeros() {
        ...
        for (int i = 0; i < mzValues.length; i++) {
            if (intensityValues[i] instanceof Double) {
                ...
                if (((Float) intensityValues[i] > 0.0f)) {
                    trimmedMzValues[trimmedSize] = mzValues[i];
                    trimmedIntensityValues[trimmedSize] = intensityValues[i];
                    trimmedSize++;
                }
            } else if (intensityValues[i] instanceof Integer) {
                if (((Integer) intensityValues[i] > 0)) {
                    trimmedMzValues[trimmedSize] = mzValues[i];
                    trimmedIntensityValues[trimmedSize] = intensityValues[i];
                    trimmedSize++;
                }
            } else if (intensityValues[i] instanceof Long) {
                if (((Long) intensityValues[i] > 0L)) {
                    trimmedMzValues[trimmedSize] = mzValues[i];
                    trimmedIntensityValues[trimmedSize] = intensityValues[i];
                    trimmedSize++;
                }
